        ...
        with_connection(pool, move |conn| {
            ...
            diesel::update(
                crate::db::schema_mysql::emoji_mappings::table.filter(
                    crate::db::schema_mysql::emoji_mappings::slack_emoji_id
                        .eq(&emoji.slack_emoji_id),
                ),
            )
            .set(changes)
            .execute(conn)
    ...
    async fn delete_emoji(&self, slack_emoji_id: &str) -> Result<(), DatabaseError> {
        ...
        with_connection(pool, move |conn| {
            diesel::delete(crate::db::schema_mysql::emoji_mappings::table.filter(
                crate::db::schema_mysql::emoji_mappings::slack_emoji_id.eq(slack_emoji_id),
            ))
            .execute(conn)
            .map(|_| ())
            .map_err(|e| DatabaseError::Query(e.to_string()))
        ...
        with_connection(pool, move |conn| {
            ...
            diesel::sql_query(
                "INSERT INTO emoji_mappings (slack_emoji_id, emoji_name, animated, mxc_url, created_at, updated_at) \
                 VALUES (?, ?, false, ?, ?, ?) \
                 ON DUPLICATE KEY UPDATE emoji_name = VALUES(emoji_name), mxc_url = VALUES(mxc_url), updated_at = VALUES(updated_at)"
            )
